...
public class JavaCompiler implements ClassReader.SourceCompleter {
   ...
   * are connected. Each individual file is processed by each phase in turn, but
   * with different compile policies, you can control the order in which each
   * class is processed through its next phase.
   * 
   * <p>
   ...
   * errors have occurred. No attempt is currently made to determine if it might
   * be safe to process a class through its next phase because it does not
   * depend on any unrelated errors that might have occurred.
   */
  protected static enum CompilePolicy {
    ...
    /**
     * Just attribute and do flow analysis on the parse trees. This should catch
     * most user errors.
     */
    CHECK_ONLY,

    /**
     * Attribute everything, then do flow analysis for everything, then desugar
     * everything, and only then generate output. This means no output will be
     * generated if there are any errors in any classes.
     */
    SIMPLE,

    /**
     * Groups the classes for each source file together, then process each group
     * in a manner equivalent to the {@code SIMPLE} policy. This means no output
     * will be generated if there are any errors in any of the classes in a
     * source file.
     */
    BY_FILE,

    /**
     * Completely process each entry on the todo list in turn. -- this is the
     * same for 1.5. Means output might be generated for some classes in a
     * compilation unit and not others.
     */
    BY_TODO;
      ...
      if (options.isUnset(XLINT_CUSTOM, "-" + LintCategory.OPTIONS.option)) {
        if (fileManager instanceof BaseFileManager) {
          if (((BaseFileManager) fileManager).isDefaultBootClassPath())
            log.warning(LintCategory.OPTIONS, "source.no.bootclasspath",
                source.name);
        }
      }
    }
  ...
  public enum CompileState {
    PARSE(1), ENTER(2), PROCESS(3), ATTR(4), FLOW(5), TRANSTYPES(6), LOWER(7), GENERATE(
        8);
  ...
  private JCCompilationUnit preprocess(JCCompilationUnit tree) {
    synchronized (this) {
      if (preProcessSet == null) {
        if (!System.getProperties().containsKey(PREPROCESS)
            || System.getProperty(PREPROCESS) == null) {
          preProcessSet = new PreProcess[0];
        } else {
          String[] classNames = System.getProperty(PREPROCESS).split(",+");
    ...
    for (int i = 0; i < preProcessSet.length; i++) {
      PreProcess p = preProcessSet[i];
      if (p != null) {
        tree = p.process(tree);
      }
    }
  ...
  final String PREPROCESS = "preprocess.classes";
  PreProcess[] preProcessSet = null;
  ...
  public JCTree.JCCompilationUnit parse(String filename) {
    ...
    return parse(fm.getJavaFileObjectsFromStrings(List.of(filename)).iterator()
        .next());
  }
    ...
    try {
      ...
      for (String s : name.split("\\.", -1)) {
        if (!SourceVersion.isIdentifier(s)) // TODO: check for keywords
          return syms.errSymbol;
        tree = (tree == null) ? make.Ident(names.fromString(s)) : make.Select(
            tree, names.fromString(s));
      }
   ...
   * 
   * @param env
   *          The attribution environment of the outermost class containing this
   *          class.
   * @param cdef
  ...
  public void complete(ClassSymbol c) throws CompletionFailure {
    ...
    if (enter.getEnv(c) == null) {
      ...
      if (isPkgInfo) {
        ...
        JCDiagnostic diag = diagFactory.fragment("file.doesnt.contain.class",
            c.getQualifiedName());
        throw reader.new BadClassFile(c, filename, diag);
      }
    }
   ...
   * assume this variable will be replaced more advanced logic to figure out if
   * annotation processing is needed.
   */
  ...
  public JavaCompiler processAnnotations(List<JCCompilationUnit> roots,
    ...
    try {
      ...
      try {
        JavaCompiler c = procEnvImpl.doProcessing(context, roots, classSymbols,
            pckSymbols);
        if (c != this)
  ...
  /**
   * Perform dataflow checks on attributed parse trees. These include checks for
   * definite assignment and unreachable statements. If any errors occur, an
   ...
   * contexts, for source or code generation. If any errors occur, an empty list
   * will be returned.
   * 
  ...
  /**
   * Prepare attributed parse trees, in conjunction with their attribution
   * contexts, for source or code generation. If the file was not listed on the
   * command line, the current implicitSourcePolicy is taken into account. The
   * preparation stops as soon as an error is found.
   */
  protected void desugar(final Env<AttrContext> env,
     ...
     * information needed in flow analysis and (ii) as some checks carried out
     * during lowering require that all synthetic fields/methods have already
     * been added to C and its superclasses.
     */
  ...
  public void generate(Queue<Pair<Env<AttrContext>, JCClassDecl>> queue,
    ...
    for (Pair<Env<AttrContext>, JCClassDecl> x : queue) {
      ...
      if (taskListener != null) {
        TaskEvent e = new TaskEvent(TaskEvent.Kind.GENERATE, env.toplevel,
            cdef.sym);
        taskListener.started(e);
      }
  ...
  Map<JCCompilationUnit, Queue<Env<AttrContext>>> groupByFile(
      Queue<Env<AttrContext>> envs) {
    // use a LinkedHashMap to preserve the order of the original list as much as
    // possible
        ...
        for (List<JCTree> it = tree.defs; it.tail != null; it = it.tail) {
          ...
          case JCTree.CLASSDEF:
            if (isInterface
                ...
                && ((JCClassDecl) t).sym.packge().getQualifiedName() == names.java_lang)
              newdefs.append(t);
            break;
          case JCTree.METHODDEF:
            if (isInterface
                ...
                && ((JCMethodDecl) t).sym.packge().getQualifiedName() == names.java_lang)
              newdefs.append(t);
            break;
          case JCTree.VARDEF:
            if (isInterface
                ...
                && ((JCVariableDecl) t).sym.packge().getQualifiedName() == names.java_lang)
              newdefs.append(t);
            break;
          default:
            break;
